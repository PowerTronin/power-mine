...
public final class PowerMineForgeAgent {
    ...
    private JsonObject health() {
        ...
        result.addProperty("minecraftVersion", "1.7.10");
    ...
    private JsonObject bridgeCapabilities() {
        JsonObject result = ok();
        result.addProperty("protocolVersion", "power-mine-agent/v1");
        result.addProperty("agentId", "power-mine-forge-1.7.10-agent");
        ...
        capabilities.addProperty("offhand", false);
        capabilities.addProperty("autoWorldOpen", true);
        capabilities.addProperty("autoWorldCreate", true);
        result.add("capabilities", capabilities);

        JsonArray limitations = new JsonArray();
        limitations.add(new JsonPrimitive("Forge 1.7.10 recipes do not have stable ids; recipe checks report the matched recipe class instead."));
    ...
    private JsonObject state() throws Exception {
        ...
        Object player = field(mc, "thePlayer", "field_71439_g");
        Object world = field(mc, "theWorld", "field_71441_e");
        JsonObject result = ok();
    ...
    private JsonObject giveItem(JsonObject body) throws Exception {
        ...
        if (slot < 0 || slot >= 40) {
            throw new IllegalArgumentException("slot must be between 0 and 39 for Forge 1.7.10");
        }
    ...
    private JsonObject placeBlock(JsonObject body) throws Exception {
        ...
        Boolean changed = (Boolean) call(world, new String[]{"setBlock", "func_147465_d"}, new Class<?>[]{int.class, int.class, int.class, blockClass(), int.class, int.class}, pos.x, pos.y, pos.z, block, metadata, 3);
        JsonObject result = ok();
    ...
    private JsonObject breakBlock(JsonObject body) throws Exception {
        ...
        Boolean changed;
        if (drop) {
            changed = (Boolean) call(world, new String[]{"destroyBlock", "func_147480_a"}, new Class<?>[]{int.class, int.class, int.class, boolean.class}, pos.x, pos.y, pos.z, true);
        } else {
            Object air = findGameObject(false, "minecraft:air");
            changed = (Boolean) call(world, new String[]{"setBlock", "func_147465_d"}, new Class<?>[]{int.class, int.class, int.class, blockClass(), int.class, int.class}, pos.x, pos.y, pos.z, air, 0, 3);
        }
        JsonObject result = ok();
        result.addProperty("changed", changed != null && changed);
        result.add("pos", blockPosJson(pos));
    ...
    private JsonObject useItem(final JsonObject body) throws Exception {
        return clientTask(new ClientTask() {
            ...
            public JsonObject run() throws Exception {
                ...
                String hand = optionalString(body, "hand", "main").toLowerCase(Locale.ROOT);
                if ("off".equals(hand) || "offhand".equals(hand) || "off_hand".equals(hand)) {
                    throw new UnsupportedOperationException("Forge 1.7.10 does not have an offhand slot");
                }

                int selectedSlot = intField(inventory, 0, "currentItem", "field_70461_c");
                Object stack = stackAt(inventory, selectedSlot);
                JsonObject before = stackJson(selectedSlot, stack, selectedSlot);
                Boolean used = Boolean.FALSE;
                if (stack != null) {
                    Object controller = field(mc, "playerController", "field_71442_b");
                    Object value = call(
                            controller,
                            new String[]{"sendUseItem", "func_78769_a", "a"},
                            new Class<?>[]{entityPlayerClass(), worldClass(), itemStackClass()},
                            player,
                            world,
                            stack
                    );
                    used = value instanceof Boolean ? (Boolean) value : Boolean.FALSE;
                    try {
                        call(player, new String[]{"swingItem", "func_71038_i", "bw"}, new Class<?>[0]);
                    } catch (Exception ignored) {
                    }
                    markInventoryDirty(inventory, player);
                }

                Object afterStack = stackAt(inventory, selectedSlot);
                JsonObject result = ok();
                result.addProperty("hand", "main");
                result.addProperty("selectedSlot", selectedSlot);
                result.addProperty("used", used.booleanValue());
                result.addProperty("screen", currentScreenName(mc));
                result.add("before", before);
                result.add("after", stackJson(selectedSlot, afterStack, selectedSlot));
                return result;
            }
        });
    }
    ...
    private JsonObject useBlock(final JsonObject body) throws Exception {
        return clientTask(new ClientTask() {
            ...
            public JsonObject run() throws Exception {
                ...
                int selectedSlot = intField(inventory, 0, "currentItem", "field_70461_c");
                Object stack = stackAt(inventory, selectedSlot);
                JsonObject beforeStack = stackJson(selectedSlot, stack, selectedSlot);
                ...
                Object value = call(
                        controller,
                        new String[]{"onPlayerRightClick", "func_78760_a", "a"},
                        new Class<?>[]{entityPlayerClass(), worldClass(), itemStackClass(), int.class, int.class, int.class, int.class, vec3Class()},
                        player,
                        world,
                        stack,
                        pos.x,
                ...
                Boolean used = value instanceof Boolean ? (Boolean) value : Boolean.FALSE;
                try {
                    call(player, new String[]{"swingItem", "func_71038_i", "bw"}, new Class<?>[0]);
                } catch (Exception ignored) {
                }
                ...
                result.add("afterStack", stackJson(selectedSlot, afterStack, selectedSlot));
                return result;
            }
        });
    }
    ...
    private JsonObject recipeCheck(JsonObject body) throws Exception {
        ...
        Object manager = callStatic(craftingManagerClass(), new String[]{"getInstance", "func_77594_a", "a"}, new Class<?>[0]);
        Object recipe = findMatchingRecipe(manager, grid, world);
        Object output = null;
        if (recipe != null) {
            output = call(recipe, new String[]{"getCraftingResult", "func_77572_b", "a"}, new Class<?>[]{craftingInventoryClass()}, grid);
        }
        if (output == null) {
            output = call(manager, new String[]{"findMatchingRecipe", "func_82787_a", "a"}, new Class<?>[]{craftingInventoryClass(), worldClass()}, grid, world);
        }
        ...
        if (expectedRecipe.length() > 0) {
            ...
            result.addProperty("recipeIdMatches", recipe != null && expectedRecipe.equals(recipe.getClass().getName()));
        }
        return result;
    }
    ...
    private JsonObject heldItemRender(final HttpExchange exchange) throws Exception {
        String requestedHand = queryString(exchange, "hand", "main").toLowerCase(Locale.ROOT);
        if ("offhand".equals(requestedHand) || "off".equals(requestedHand)) {
            throw new UnsupportedOperationException("Forge 1.7.10 does not have an offhand slot");
        }
        ...
        return clientTask(new ClientTask() {
            ...
            public JsonObject run() throws Exception {
                ...
                result.addProperty("hand", "main");
                result.addProperty("selectedSlot", selectedSlot);
                result.add("stack", stackJson(selectedSlot, stack, selectedSlot));
    ...
    private JsonObject playerJson(Object player) throws Exception {
        JsonObject result = new JsonObject();
        result.addProperty("name", stringCall(player, "", "getCommandSenderName", "func_70005_c_"));
    ...
    private JsonObject targetJson(Object mc, Object world) throws Exception {
        ...
        int x = intField(target, 0, "blockX", "field_72311_b");
        int y = intField(target, 0, "blockY", "field_72312_c");
        int z = intField(target, 0, "blockZ", "field_72309_d");
        if (x != 0 || y != 0 || z != 0) {
            BlockPos pos = new BlockPos(x, y, z);
            result.add("pos", blockPosJson(pos));
            JsonObject block = blockJson(world, pos);
    ...
    private JsonObject inventorySummary(Object inventory) throws Exception {
        ...
        int occupied = 0;
        for (int index = 0; index < 40; index++) {
            if (stackAt(inventory, index) != null) {
                occupied++;
            }
        }
        result.addProperty("slots", 40);
    ...
    private JsonObject stackJson(int index, Object stack, int selectedSlot) {
        ...
        result.addProperty("empty", stack == null);
        if (stack != null) {
            try {
    ...
    private JsonObject blockJson(Object world, BlockPos pos) throws Exception {
        Object block = call(world, new String[]{"getBlock", "func_147439_a"}, new Class<?>[]{int.class, int.class, int.class}, pos.x, pos.y, pos.z);
        Integer metadata = (Integer) call(world, new String[]{"getBlockMetadata", "func_72805_g"}, new Class<?>[]{int.class, int.class, int.class}, pos.x, pos.y, pos.z);
    ...
    private Object requirePlayer() throws Exception {
        Object player = field(requireMinecraft(), "thePlayer", "field_71439_g");
        if (player == null) {
    ...
    private Object requireWorld() throws Exception {
        Object world = field(requireMinecraft(), "theWorld", "field_71441_e");
        if (world == null) {
            throw new IllegalStateException("Minecraft client is not in a loaded world");
        }
        return world;
    ...
    private Object stackAt(Object inventory, int slot) throws Exception {
        Object array = slot < 36 ? field(inventory, "mainInventory", "field_70462_a") : field(inventory, "armorInventory", "field_70460_b");
        int offset = slot < 36 ? slot : slot - 36;
    ...
    private Object vec3(double x, double y, double z) throws Exception {
        return callStatic(
                vec3Class(),
                new String[]{"createVectorHelper", "func_72443_a", "a"},
    ...
    private Object findMatchingRecipe(Object manager, Object grid, Object world) throws Exception {
        Object recipes = call(manager, new String[]{"getRecipeList", "func_77592_b", "b"}, new Class<?>[0]);
    ...
    private Object findGameObject(boolean item, String id) throws Exception {
        String[] pieces = splitIdentifier(id);
        Class<?> gameRegistry = Class.forName("cpw.mods.fml.common.registry.GameRegistry");
        String method = item ? "findItem" : "findBlock";
        return callStatic(gameRegistry, new String[]{method}, new Class<?>[]{String.class, String.class}, pieces[0], pieces[1]);
    }
    ...
    private String registryName(Object object, boolean item) {
        ...
        try {
            ...
            Object uniqueName = callStatic(gameData, new String[]{"getUniqueName"}, new Class<?>[]{item ? itemClass() : blockClass()}, object);
            if (uniqueName != null) {
        ...
        try {
            Class<?> gameData = Class.forName("cpw.mods.fml.common.registry.GameData");
            Object registry = callStatic(gameData, new String[]{item ? "getItemRegistry" : "getBlockRegistry"}, new Class<?>[0]);
            Object name = call(registry, new String[]{"getNameForObject"}, new Class<?>[]{Object.class}, object);
    ...
    private Class<?> vec3Class() throws ClassNotFoundException {
        return classByNameAny("net.minecraft.util.Vec3", "azw");
    }
    ...
    private Class<?> screenshotHelperClass() throws ClassNotFoundException {
        return classByNameAny("net.minecraft.util.ScreenShotHelper", "bbp");
    }
    ...
    private Class<?> gameTypeClass() throws ClassNotFoundException {
        return classByNameAny("net.minecraft.world.WorldSettings$GameType", "ahj$a");
    }
